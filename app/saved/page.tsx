...
/* 40×40 unsave button — proper tap target, no bare emoji */
function UnsaveBtn({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} title="Unsave"
      className="flex items-center justify-center rounded-xl flex-shrink-0"
      style={{ width: 40, height: 40, fontSize: 18, background: "rgba(239,68,68,0.08)" }}>
      ❤️
    </button>
  );
}
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      try {
        ...
        // network error — stay authed, show empty
      } finally {
        setLoading(false);
      }
    });
    ...
    <AppShell activeTab="saved">
      <div className="flex flex-col h-full" style={{ background: "var(--bg)" }}>
        <div className="flex-shrink-0 px-4 pt-4 pb-3"
          ...
          <h1 className="font-syne font-black text-xl" style={{ letterSpacing: "-0.4px" }}>Saved</h1>
        </div>
        ...
        <div className="flex-1 overflow-y-scroll scroll-none px-4 py-3">
          ...
          {authed && !loading && favorites.map((fav) => {
            ...
            if (shop) return (
              <div key={fav.id} className="flex items-center gap-3 px-3.5 rounded-2xl mb-3"
                style={{ background: "rgba(255,255,255,0.034)", border: "1px solid rgba(255,255,255,0.07)", minHeight: 64 }}>
                <Link href={`/shop/${shop.slug}`} className="flex items-center gap-3 flex-1 min-w-0 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-0.5" style={{ color: "var(--t3)" }}>
                      {shop.avg_rating > 0 ? `★ ${shop.avg_rating} · ` : ""}{shop.locality?.name}
                    </p>
                  </div>
                </Link>
                <UnsaveBtn onClick={() => unsave(fav)} />
              </div>
            );

            if (offer) return (
              <div key={fav.id} className="flex items-center gap-3 px-3.5 rounded-2xl mb-3"
                style={{ background: "rgba(255,94,26,0.06)", border: "1px solid rgba(255,94,26,0.16)", minHeight: 64 }}>
                <Link href={`/shop/${offer.shop?.slug}`} className="flex items-center gap-3 flex-1 min-w-0 py-3">
            ...
            if (locality) return (
              <div key={fav.id} className="flex items-center gap-3 px-3.5 rounded-2xl mb-3"
                style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.16)", minHeight: 64 }}>
