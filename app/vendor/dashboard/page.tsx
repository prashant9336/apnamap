    ...
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <div className="sticky top-0 z-50 flex items-center gap-3 px-4 py-3"
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px]" style={{ color: "var(--t3)" }}>Manage your shops & offers</p>
        </div>
        <Link href="/vendor/onboarding"
          className="px-3 py-1.5 rounded-full text-xs font-bold text-white"
          style={{ background: "var(--accent)" }}>+ Add Shop</Link>
      </div>
      ...
      <div className="px-4 py-4">
        ...
        {shops.length === 0 && (
          <div className="text-center py-12 rounded-2xl"
            ...
            <p className="text-sm mb-4" style={{ color: "var(--t2)" }}>Add your first shop to start getting customers</p>
            <Link href="/vendor/onboarding"
              className="px-6 py-2.5 rounded-full text-sm font-bold text-white"
              style={{ background: "var(--accent)" }}>Add My Shop</Link>
          </div>
        )}
          ...
          {shops.map((shop: any) => (
            <div key={shop.id} className="p-4 rounded-2xl"
              ...
              <div className="flex items-start justify-between gap-2 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "var(--t3)" }}>{shop.locality?.name}</p>
                  </div>
                </div>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${shop.is_approved ? "" : "opacity-75"}`}
              ...
              <div className="flex items-center gap-4 mb-3 text-xs" style={{ color: "var(--t2)" }}>
                <span>🎯 {shop.offers?.filter((o: any) => o.is_active).length ?? 0} active offers</span>
                <span>⭐ {shop.avg_rating.toFixed(1)} ({shop.review_count} reviews)</span>
              </div>

              <div className="flex gap-2">
                <Link href={`/vendor/shop?id=${shop.id}`}
                ...
                <Link href={`/vendor/offers?shop_id=${shop.id}`}
                  className="flex-1 py-2 rounded-lg text-xs font-semibold text-center"
