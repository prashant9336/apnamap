...
export default function LoginForm({ redirectTo }: { redirectTo: string }) {
  ...
  // No useEffect session check — server component already handles redirect
  // for authenticated users before this form is ever rendered.
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (err) {
      setError(mode === "mobile"
        ? "Wrong mobile number or password. Have you set your password yet?"
        : err.message);
      setLoading(false); return;
    }

    // Vendor password-change gate
    const { data: vendorRow } = await sb.from("vendors")
      .select("must_change_password").eq("id", data.user.id).maybeSingle();
    ...
    <div style={S.pg}>
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 24, background: "rgba(255,255,255,0.05)", borderRadius: 12, padding: 4 }}>
          {([{ id: "mobile", label: "📱 Mobile" }, { id: "email", label: "✉️ Email" }] as const).map(opt => (
            <button key={opt.id} type="button" onClick={() => { setMode(opt.id); setError(""); }}
              style={{ flex: 1, padding: "9px", borderRadius: 9, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", background: mode === opt.id ? "#FF5E1A" : "transparent", color: mode === opt.id ? "#fff" : "rgba(255,255,255,0.45)", transition: "all 0.15s" }}>
              {opt.label}
            </button>
          ))}
        </div>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 9 }}>
                ...
                <input type="tel" inputMode="numeric" value={mobile}
                  ...
                  placeholder="10-digit number" style={{ ...S.input, flex: 1 }} required autoFocus />
              </div>
            </div>
          ) : (
            <div>
